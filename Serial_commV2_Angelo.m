...
s = serialport('COM4',115200,'Timeout',3);
pause(1);
getpinstatus(s)

%Add the UserData Field to the serial port object
s.UserData = struct("Pos",[],"Prec",[],"Time",[],"Count",1,"Stop",0);
...
sec = 120;      %Length of acquisition in s
Fs = 10;
data = readline(src);
...
%s.UserData.Time(end)-s.UserData.Time(1)
%s.UserData.Stop = 1
%r = squeeze(s.UserData.Pos(:,1,:));